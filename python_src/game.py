...
class Game:
	...
	# don't think my if statment will always be true cause what if the big blind 
	#just does the min bet then bet will be less than min bet
	#so if the big blind bets min and small blind just calls this will never trigger
	#Not sure if that would screw things up need to test 
	#think you can just do it to where if the total is greater than 2 bb's or maybe just if it's 1 or greater
	#pretty sure 
	def street_preflop(self):
		...
		min_bet = 2 * self.bb
		collect_sb = True
		#Think this should be while current < num_player
