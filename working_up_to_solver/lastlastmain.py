class Kuhn:
    ...
    def cfr_fta(self,card):
        ...
        for acard in self.cards:
            if acard != card:
                sta = self.sta_strat[card]
                check_check = sta[0] if acard > card else -1 * sta[0]
                check_bet_fold = -1 * fta[2]
                check_bet_call = fta[3] * 2 if card> acard else -2 * fta[3]
                bet_fold = -1 * fta[1]
                bet_call = fta[1] * 2 if card > acard else -2 * fta[1]
